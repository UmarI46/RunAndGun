...
current_user=guiLI.Nselect
cursor.execute("SELECT Score FROM Scoreboard WHERE UsernameS='{}'".format(current_user))
stored_score=cursor.fetchall()
...
class Player(pygame.sprite.Sprite):
    ...
    def Shoot(self):
        #power shot working
        if self.Pshooting==True:
            self.RBsize=50
            LevelLoader.Pdmg=3
            #print('big bullet power up on')
            if frame%350==0:
                LevelLoader.Pdmg=1
                self.RBsize=15
                #print('big bullet power up off')
                self.Pshooting=False
        ...
        if keyP[pygame.K_SPACE]:
            ...
            if self.shoot1==False and self.Cshoot==False:
                self.bullet1=pygame.Rect((self.player.x+64),self.player.y+75,self.RBsize,self.RBsize)
                self.hold=0
                if frame %11==0:
                    self.shoot1=True
            elif self.shoot1==True and self.shoot2==False and self.Cshoot==False:
                self.bullet2=pygame.Rect((self.player.x+64),self.player.y+75, self.RBsize,self.RBsize)
                self.hold=0
                if frame %11==0:
                    self.shoot2=True
            elif self.shoot2==True and self.shoot3==False and self.Cshoot==False:
                self.bullet3=pygame.Rect((self.player.x+64),self.player.y+75, self.RBsize,self.RBsize)
                self.hold=0
                if frame %11==0:
                    self.shoot3=True
    ...
    def Damage(self):
        ...
        if self.invin==True:
            #print('invincibility power up on')
            ouchRect=pygame.Rect(self.player.x,self.player.y, 64,128)
            pygame.draw.rect(Surface,YELLOW,ouchRect)
            self.hurt=True
            if frame%350==0:
                #print('invincibility power up off')
                self.invin=False        
...
class Boss(pygame.sprite.Sprite):
    def __init__(self,x,y):
        ...
        self.Lleg.x= x
        self.Lleg.y= HEIGHT-tile_size*4
        ...
        self.hit=False
        self.upperLim=10
        self.action=random.randint(1,self.upperLim)
        self.Mattack=False
        self.Sattack=False
        self.Aattack=False
        self.dead=False
        ...
        for index in range (len(self.parts)):
            if playerChar.bullet1 != None and self.hurt==False:
                if playerChar.bullet1.colliderect(self.parts[index]):
                    self.HP-=self.Pdmg
                    #print('Player damage: ' +str(self.Pdmg))
                    playerChar.bullet1.x=WIDTH
                    self.hurt=True
                    #boss dies
                    if self.HP<=0:
                        if playerChar.lives<2:
                            playerChar.Cscore+=1000
                        elif playerChar.lives>4 or playerChar.HP>=20:
                            playerChar.Cscore+=2000
                        else:
                            playerChar.Cscore+=1500
            ...
            if playerChar.Cbullet != None and self.hurt==False:
                if playerChar.Cbullet.colliderect(self.parts[index]):
                    self.HP-=3
                    #print('Player charged damage: ' +str(self.Pdmg))
            ...
            #boss gets hurt
            if self.hurt==True:
                ouchRectL=pygame.Rect(self.Lleg.x,self.Lleg.y, tile_size,tile_size*2)
                ouchRectR=pygame.Rect(self.Rleg.x,self.Rleg.y, tile_size,tile_size*2)
                ouchRectB=pygame.Rect(self.body.x,self.body.y, tile_size*4,tile_size*3)
                ouchRectA=pygame.Rect(self.arm.x,self.arm.y, tile_size*3,tile_size)
                ouchRectH=pygame.Rect(self.head.x,self.head.y, tile_size*2,tile_size*2)
                pygame.draw.rect(Surface,BRED,ouchRectL)
                pygame.draw.rect(Surface,BRED,ouchRectR)
                pygame.draw.rect(Surface,BRED,ouchRectB)
                pygame.draw.rect(Surface,BRED,ouchRectA)
                pygame.draw.rect(Surface,BRED,ouchRectH)
                if frame%90==0:
                    self.hurt=False

            #player stands in boss
            if self.parts[index].colliderect(playerChar.player) and playerChar.hurt==False:
                playerChar.HP-=self.Bdmg
                #print('Body damage: ' +str(self.Bdmg)) 
                playerChar.hurt=True

            if playerChar.died==True:
                self.HP=26
    ...
    def Stomp(self,playerChar):
        ...
        if self.jumping==True and self.Lleg.bottom<=0:
            ...
            warningRect=pygame.Rect(self.arm.x,self.Lleg.bottom+tile_size*10, tile_size*6,tile_size)
            ...
            if self.tracking==True:
                ...
                if self.Lleg.x< playerChar.player.x:
                    self.Lleg.x+=10
                #gives time for the tracking to occurself.action
                if frame%320==0:
                    self.tracking=False
    ...
    def ArmSlam(self,playerChar):
        ...
        if self.tracking==True:
            ...
            if self.arm.x +96< playerChar.player.x:
                ...
                if frame%10==0 and self.tracking==True:
                    self.tracking=False
            
                
        #gives time for the player to move out of the way
        if frame%30==0 and self.tracking==False and self.hit==False:
            self.stomping=True
        ...
        if self.stomping==True and self.tracking==False:
            ...
            if self.arm.bottom>=HEIGHT-128:   
                dustRect=pygame.Rect(self.arm.x-tile_size,self.arm.bottom, tile_size*5,8)
                pygame.draw.rect(Surface,WHITE,dustRect)
                if dustRect.colliderect(playerChar.player) and playerChar.hurt==False:
                    playerChar.HP-=round(self.Bdmg/2)
                    #print('Dust damage: ' +str(round(self.Bdmg/2)))
                    playerChar.player.bottom=tile_size*2
                    playerChar.hurt=True
                if playerChar.hurt==True or frame%30:
                    self.hit=True
                    self.stomping=False
    ...
    def MissielsAttack(self,playerChar):
        ...
        if self.m1Track==True:
            ...
            if self.mRect1.x< playerChar.player.x:
                 self.mRect1.x+=4
            wRect1=pygame.Rect(self.mRect1.x,self.mRect1.y+tile_size*13, 32,32)
        ...
        if self.m2Track==True:
            ...
            if self.mRect2.x< playerChar.player.x:
                 self.mRect2.x+=4
            wRect2=pygame.Rect(self.mRect2.x,self.mRect2.y+tile_size*13, 32,32)
        ...
        if self.m3Track==True:
            ...
            if self.mRect3.x< playerChar.player.x:
                 self.mRect3.x+=4
            wRect3=pygame.Rect(self.mRect3.x,self.mRect3.y+tile_size*13, 32,32)
        ...
        if self.m4Track==True:
            ...
            if self.mRect4.x< playerChar.player.x:
                 self.mRect4.x+=4
            wRect4=pygame.Rect(self.mRect4.x,self.mRect4.y+tile_size*13, 32,32)
        ...
        if self.m5Track==True:
            ...
            if self.mRect5.x< playerChar.player.x:
                 self.mRect5.x+=4
            wRect5=pygame.Rect(self.mRect5.x,self.mRect5.y+tile_size*13, 32,32)
        ...
        if self.mRect1.colliderect(playerChar.player) and playerChar.hurt==False:
            playerChar.HP-=self.Mdmg
            #print('Missile damage: ' +str(self.Mdmg))
            playerChar.hurt=True
    ...
    def ScoreTot(self,playerChar):
        ...
        if self.dead==True:
            ...
            #replaces the old score with the better one.
            database.commit()
            if (stored_score[0][0])< playerChar.Cscore:
                cursor.execute("UPDATE Scoreboard SET Score={} WHERE UsernameS='{}'".format(playerChar.Cscore,current_user))
                cursor.execute("SELECT * FROM Scoreboard WHERE UsernameS='{}'".format(current_user))
                database.commit()
    ...
    def update(self,playerChar,LevelLoader):
        if self.HP<=0:
            GameOver=STAT_FONT.render('YOU WON!!!!!',5, BLACK)
            Surface.blit(GameOver,(WIDTH/2,HEIGHT/2))
        if counter==3 and self.HP>0 and LevelLoader.flagGet==False:

            #attack radomizer
            if self.HP> 14 and self.HP<20:
                self.upperLim=7
            elif self.HP<14:
                self.upperLim=5
            ...
            #attacks occur
            if frame%30==0 and self.attacking==False:
                self.action=random.randint(1,self.upperLim)
            ...
            if self.action==1 and self.attacking==False:
                ...
                #print('Chance of attack: ' +str(self.upperLim))

            if self.action==2 and self.attacking==False or playerChar.player.x>self.body.right:
                self.jumping=True
                self.Sattack=True
                #print('Boss HP: ' +str(self.HP))
                #print('Chance of attack: ' +str(self.upperLim))
...
class Level:
    def __init__(self,level_data,surface):
        ...
        self.fall=False
        self.upperLim=10
        self.HPspawn=random.randint(1,self.upperLim)
        self.flagGet=False
        #turret intit
    ...
    def setup_level(self,layout):
        ...
        self.powerShots=pygame.sprite.Group()
        ...
        for row_index,row in enumerate(layout):
            for col_index,col in enumerate(row):
                ...
                if col== 'Z':
                    x=col_index * 64
                    y=row_index *64
                    tile=Tile((x,y),32,PowerShot)
                    self.powerShots.add(tile)
    ...
    def x_collison(self,playerChar):
        ...
        for sprite in self.tilesx.sprites():
            ...
            #checks if a stab bot is in the wall
            for Sbot in self.StabBots.sprites():
                if sprite.rect.colliderect(Sbot.rect):
                    self.Sdetect=True

            #checks if a gun bot is in the wall
            for Gbot in self.GunBots.sprites():
                if sprite.rect.colliderect(Gbot.rect):
                    self.Gdetect=True
    ...
    def TurretStuff(self,playerChar):
        ...
        for sprite in self.turrets.sprites():
            #difficulty scalling
            if playerChar.lives==4:
                self.Tdmg=2
                self.upperLim=15
            elif playerChar.lives>4 or playerChar.HP>=20:
                self.Tdmg=3
                self.upperLim=15
            elif playerChar.lives<2:
                self.Pdmg=2
                self.upperLim=5
                #default
            elif playerChar.lives==2 or playerChar.lives==3 and playerChar.HP<=20:
                self.Tdmg=1
                self.Pdmg=1
                self.upperLim=10
            ...
            if self.S_bullet1== True:
                ...
                if self.bullet1.colliderect(playerChar.player) and playerChar.hurt==False:
                    playerChar.HP-=self.Tdmg
                    #print('Turret damage:' +str(self.Tdmg))
                    playerChar.hurt=True
                    self.S_bullet1=False
            ...
            if playerChar.bullet1 != None and self.Thurt==False:
                if playerChar.bullet1.colliderect(sprite.rect):
                    ...
                    if self.THP<=0:
                        ...
                        self.THP=3
                        #print('Power up chance:' +str(self.upperLim))
                        self.HPspawn=random.randint(1,self.upperLim)
                        if self.HPspawn==1:
                            tile=Tile((sprite.rect.x,sprite.rect.y+tile_size),32,healthUp)
                            self.healthUps.add(tile)
                        self.HPspawn=random.randint(1,self.upperLim)
        ...
        for sprite in self.GunBots.sprites():
            ...
            if self.S_Gbullet1== True:
                ...
                if self.Gbullet1.colliderect(playerChar.player) and playerChar.hurt==False:
                    playerChar.HP-=self.Gdmg
                    #print('Gun Bot damage:' +str(self.Gdmg))
                    playerChar.hurt=True
                    self.S_Gbullet1=False
                #bullet reaches limit
            ...
            if self.Gdetect==True and frame%1000==0:
    ...
    def StabBotStuff(self,playerChar):
        for sprite in self.StabBots.sprites():
            ...
            if playerChar.lives<=2:
                ...
                self.Sdmg=4
                self.upperLim=10
            elif playerChar.lives==3 or playerChar.lives==4 and playerChar.HP<=20:
                self.Sdmg=3
                self.Pdmg=1
                self.upperLim=10
            elif playerChar.lives<2 and playerChar.HP<=20:
                self.Pdmg=2
                self.Sdmg=2
                self.upperLim=3
            
            #stab bot moving
            if playerChar.player.x > sprite.rect.x - tile_size *8 and self.stab==False:
                sprite.rect.x-=5
            if sprite.rect.x< playerChar.player.x and self.stab==False:
                sprite.rect.x+=10
            #enemy does damage
            if sprite.rect.colliderect(playerChar.player) and playerChar.hurt==False:
                playerChar.HP-=self.Sdmg
                #print('Stab Bot damage:' +str(self.Sdmg))
                playerChar.hurt=True
                self.stab=True
            #step back
            ...
            #dissapear after time
            if self.Sdetect==True and frame%1000==0:
            ...
            if playerChar.bullet1 != None and self.Shurt==False:
                if playerChar.bullet1.colliderect(sprite.rect):
                    ...
                    playerChar.bullet1.x=WIDTH
                    #print('Player damage:' +str(self.Pdmg))
                    #print('Stab Bot HP:' +str(self.SHP))
                    self.Shurt=True
                    #stab bot dies
                    if self.SHP<=0:
    ...
    def run(self,playerChar):
        ...
        if playerChar.player.x >= scrollThresh + tile_size *2 and playerChar.moveRight==True:
            ...
            self.powerShots.update(self.scrollSpeed)
        ...
        if self.flagGet==True:
            Flagtxt=STAT_FONT.render('Press "Q" to proceed.',5, BLACK)
            Surface.blit(Flagtxt,(WIDTH/2,HEIGHT/2))
        ...
        #power shot getting
        for sprite in self.powerShots.sprites():
            if sprite.rect.colliderect(playerChar.player):
                playerChar.Cscore+=100
                playerChar.Pshooting=True
                self.powerShots.remove(sprite)
        ...
        self.powerShots.draw(self.display_surface)
...
while running:
    ...
    if key[pygame.K_q] and LevelLoader.flagGet==True:
        LevelLoader=Level(AllLevels[counter],Surface)
        TempScore=playerChar.Cscore
        LevelLoader.flagGet==False
        #print('get the next flag!')
